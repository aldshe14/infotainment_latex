%Base ordner for all other ordners (figures, appendix, ....)
\ifdefined\bordner
\renewcommand{\bordner}{./../50_aldosheldija/}
\else
\newcommand{\bordner}{./../50_aldosheldija/}
\fi


%author of the document
\ifdefined\autor
\renewcommand{\autor}{Aldo Sheldija}
\else
\newcommand{\autor}{Aldo Sheldija}
\fi

%titelpage first Line
\ifdefined\titelf
\renewcommand{\titelf}{Diplomarbeit}
\else
\newcommand{\titelf}{Diplomarbeit}
\fi


%Name of Chapter
\ifdefined\docname
\renewcommand{\docname}{Admin Webseite - Aldo Sheldija}
\else
\newcommand{\docname}{Admin Webseite - Aldo Sheldija}
\fi



%titelpage secound Line
\ifdefined\titels
\renewcommand{\titels}{\docname}
\else
\newcommand{\titels}{\docname}
\fi

%Check Ordner for Figures
\ifdefined\ordnerfigures
\renewcommand{\ordnerfigures}{\bordner figures/}
\else
\newcommand{\ordnerfigures}{\bordner figures/}
\fi

%Check Ordner for Appendix
\ifdefined\ordnerappendix
\renewcommand{\ordnerappendix}{\bordner appendix/}
\else
\newcommand{\ordnerappendix}{\bordner appendix/}
\fi

%Check Ordner for Sourcecode
\ifdefined\ordnersource
\renewcommand{\ordnersource}{\bordner source/}
\else
\newcommand{\ordnersource}{\bordner source/}
\fi